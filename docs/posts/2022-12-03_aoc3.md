...
And, for some reason, my intuition also thinks sets are really smart
...
turning it into an array of strings), while the 'naive', 'dumb', more
...
this embarrassment. For comparison, here's the cpu and mem profiles of
...
Nothing much to say here. 70% of the time is spent comparing characters,
which is the core of what we want to do, anyway, and not much extra
memory consumption compared to the input file size. For extra fun, here
are the svg files `pprof` can generate, for the slow version:
...
answers for you, dear reader. It might even be an uncomfortable answer
...
for days one and two, even though I couldn't resist the temptation of
...
[^1]: Assuming lookup is O(1)... approximately. Of course, it's more
    ...
    more like O(n^2) (maybe O(n log n) using unsorted trees). I bet all
